...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query_check_player = "SELECT COUNT(*) AS num_players FROM event_players WHERE licenseNum = ? AND supervisor_id = ?";
    $stmt_check_player = mysqli_prepare($con, $query_check_player);

    if ($stmt_check_player) {
        mysqli_stmt_bind_param($stmt_check_player, "si", $licenseNum, $coach_id);
        ...
        if ($num_players > 0) {
            ...
            $query_insert_player = "INSERT INTO event_players (firstName, lastName, height, licenseNum, supervisor_id) VALUES (?, ?, ?, ?, ?)";
            $stmt_insert_player = mysqli_prepare($con, $query_insert_player);

            if ($stmt_insert_player) {
                // Bind parameters including the coach's ID
                mysqli_stmt_bind_param($stmt_insert_player, "ssisi", $firstName, $lastName, $height, $licenseNum, $coach_id);
                // Execute the query
