    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                ...
                //$_SESSION['club_name'] = $clubRow['clubname']; // Remove this line
                //echo $_SESSION['supervisor_id']; 
                // Check if the user is an admin
                if ($email == "admin@gmail.com" && $password == "admin2003") {
                    header("Location: admine.php");
                    exit();
                } else {
                    if ($row['approved'] == 1) {
                        // User's registration is approved, proceed to dashboard
                        header("Location: dash.php");
                        exit();
                    } else {
                        // User's registration is not approved, display message
