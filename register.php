...
// Your code to create the userss1 table
...
$dbname = "bd3"; // Define the database name here

// Check if the 'approved' column exists before attempting to add it
$query_check = "SELECT * FROM information_schema.columns WHERE table_schema = '$dbname' AND table_name = 'userss1' AND column_name = 'approved'";
...
if (mysqli_num_rows($result_check) == 0) {
    // The 'approved' column doesn't exist, so add it
    $query_add_column = "ALTER TABLE userss1 ADD COLUMN approved BOOLEAN DEFAULT 0";
    $result_add_column = mysqli_query($con, $query_add_column);

    if (!$result_add_column) {
        die("Error altering table: " . mysqli_error($con));
    }
} else {
    // The 'approved' column already exists
    echo "The 'approved' column already exists in the table.";
}
